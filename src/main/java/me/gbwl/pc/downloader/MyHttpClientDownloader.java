...
public class MyHttpClientDownloader extends AbstractDownloader {
    ...
    public Page download(Request request, Task task) {
        ...
        try {
            ...
            Map<String, String> cks = site.getCookies();
            if (cks != null && cks.size() > 0) {
            	for (Map.Entry<String, String> entry : cks.entrySet()) {
            		String[] arrs = entry.getValue().split(";");
            		BasicClientCookie2 c = new BasicClientCookie2(entry.getKey(), arrs[0]);
            		Date date = new Date(arrs[1]);
        			c.setExpiryDate(date);
        			c.setPath("/");
        			c.setDomain("www.ccdi.gov.cn");
